...
url='/Users/dennie/Desktop/NEU/ALY 6070/Rgraphics (1)/dataSets/landdata-states.csv'
...
rec.res <- data.frame(received = c(92000,105800,126960,166290,102810,124200,111090
                                   , 91080, 139380, 110400, 95910, 102810),
                      resolved = c(92000, 105800, 126960, 163530, 102210, 124890
                                   , 103500, 84870, 107640, 86940, 71760, 85560),
                      month = c("jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec")
                      )
for.chart <- melt(rec.res)

for.chart[,"month"] <- factor(for.chart[,"month"]
                              , levels = c("jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec")
                              )

##' Original chart
##' 
ggplot(for.chart) + geom_bar(aes(x=month,y=value, fill = variable),stat="identity", position="dodge") +
                    geom_text(aes(x=month,y=value+1000, label = value),check_overlap = TRUE,position = position_dodge(.9))

##' Building new chart
##' 
p <- ggplot(for.chart) + geom_line(aes(x=month,y=value, color = variable, group=variable)
                                   , size = 3)

p <- p +   scale_y_continuous(labels = paste0(c(75,100,125,150), "K"),
                              breaks = 10^3 * c(75,100,125,150))

p <- p + labs(title  = "Please approve replacements for the 3 agents who were fired in June")

p <- p + theme_minimal() + theme(panel.grid = element_blank()
...
p <- p + geom_vline(aes(xintercept = which(levels(month) %in% 'jun')))

p <- p + geom_text(data = data.frame(),
                   aes( x=c("jun")
                        ,y =c(160000)
                        , label = paste0("After 3 agents were let go in June,", "\n",
